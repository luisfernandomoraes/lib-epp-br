...
unique_ptr<DomParser> _command_parser;

void connect()
{
    printf("Connecting to the server....");
    try {
        _session->connect("../certs/client-pwd.pem",
                          "../certs/root.pem",
                          "shepp");
...
int main(int argc, char **argv)
{
    ...
    int port = 700;
    const string server = "beta.registro.br";
    try {

        _session = unique_ptr<Session>(new Session(server, port));
